...
def genNonLoser(T, player):
   ...
   pos = 0
   for num in test_T:
      if num == 0 and player == 1:
         test_T[pos] = 2
         if analyzeBoard(test_T) == 2:
            return pos
         else:
            test_T = list(T)
      if num == 0 and player == 2:
         test_T[pos] = 1
         if analyzeBoard(test_T) == 1:
            return pos
         else:
            test_T = list(T)
      pos += 1
...
def genRandomMove(T, player):
   ...
   if analyzeBoard(test_T) == 0 and test_T[random_index] == 0:
      test_T[random_index] = player
      if analyzeBoard(test_T) == -1:
         return -1
      else:
         return random_index
   elif analyzeBoard(test_T) == 0:
      while test_T[random_index] != 0:
         random_index = random.randint(0, 8)
      return random_index
   else:
      return -1
